...
Public Class IntegrationParser
    ...
    Sub ParseVerintOnyxEmail(ByVal TicketEmailBody As String, ByVal MsgID As String, ByVal IntegrationRequestType As String, ByRef TakeABreak As Boolean)
        ...
        Try
            ...
            REM if a dash exists then after the dash there is the Site Name.  Before the dash it is the Company Name
            If PositionDash Then
                SiteName = Trim(Right(CompanyName, Len(CompanyName) - PositionDash))
                CompanyName = Trim(Left(CompanyName, PositionDash - 1))
            End If
            If Debug_Mode Then
                    ...
                    Select Case sAction
                        Case "A"
                            ...
                            sql(1) = "INSERT INTO MessageIDinProgress " _
                                        ...
                                        & "GetDate() " _
                                        & ");"
                            If Debug_Mode Then sEventEntry = sEventEntry & "UpdateMessageIDinProgress returns: " _
                        ...
                        Case "D"
                            ...
                            .ExecuteSQL(sql(0))
                    End Select
                    If Debug_Mode Then WriteToEventLog(sEventEntry, EventLogEntryType.Information)
                End With
            End If
        Catch ex As Exception
            TakeABreak = True
    ...
    Private Function ExtractSingleLineData(ByVal sBody As String, ByVal sFind As String, ByRef sFound As String, ByRef iStart As Integer, ByRef iEnd As Integer) As Boolean
        ...
        REM Dim sNext As String -- for debugging
        ...
        REM initializing Bookmark for every e-mail extraction
        If iStart = 0 Or iEnd = 0 Then Bookmark = 1

        REM reset sFound for every round
        sFound = ""
        ...
        If iStart > 0 Then
            ...
            End If
            If iStart > 0 Then
                REM LTrim space bar and Tab, but must not LTrim Cr, Lf, Crlf, or NewLine 
                REM Otherwise, if there is Cr, Lf, Crlf, or NewLine immediately after colon, 
                REM     it causes a bug, i.e. retrieving over the next data field 
            ...
            End If
        End If
        If iStart > 0 And iEnd > 0 And iEnd - iStart + 1 >= 0 Then
            Bookmark = iEnd + 1
    ...
    Private Function ExtractWorkNotes(ByVal sBody As String, ByVal sFind As String, ByRef sFound As String, ByRef iStart As Integer, ByRef iEnd As Integer) As Boolean
        sFound = ""
        iStart = InStr(iEnd, sBody, "WorkNotes")
        iEnd = Len(sBody)
        If iStart > 0 Then
            iStart = InStr(iStart + 9, sBody, ":")
            REM if cannot locate a colon, then returns sFound as empty string
            If iStart > 0 Then
                iStart = iStart + 1
                If iEnd >= iStart Then
                    sFound = Mid(sBody, iStart, iEnd - iStart)
