...
Public Class IntegrationParser
    ...
    Public Shared Debug_Mode As Boolean = True
    ...
    Public Function ProcessEmailInbox() As Boolean
        ...
        Dim TakeABreak As Boolean   ' Adam Ip 2011-08-11
        ...
        If Debug_Mode Then WriteToEventLog("CIM Service Started processing - UK US", EventLogEntryType.Information)
        ...
        For i = 0 To MAX_AREA - 1
            ...
            Try
                ...
                Else
                    ...
                    If OpenDB() = True And TakeABreak = False Then
                        ...
                        For Each msg In Inbox(i)
                            ...
                            REM No Message ID found, then this is a new message, so process.  If Message ID found, then no process.
                            If Not FindMessageID(msg.MessageID, TakeABreak) Then
                                If TakeABreak = False Then
                                    sEventEntry = "CIM Service: (" & DataAreaID & ") Procedure: ProcessEmailInbox - Started processing Message ID: " _
                                        & msg.MessageID & vbCrLf _
                                    ...
                                    If ValidateEmailSource(msg.MessageID, msg.From.ToString()) = False Then
                                        If UpdateMessageID(msg.MessageID) = False Then TakeABreak = True
                                    ElseIf ProcessSupportMessage(msg, DataAreaID) = True Then
                                        If UpdateMessageID(msg.MessageID) = False Then TakeABreak = True
                                    End If
                                End If
                            End If
            ...
            Catch ex As Exception
                ...
                sEventEntry = "CIM Service : (" & DataAreaID & ") Function: ProcessEmailInbox, returns " _
                    & Convert.ToString(ProcessEmailInbox) & vbCrLf & "Exception: " & ex.Message
    ...
    Private Function ProcessSupportMessage(ByVal msg As MailMessage, ByVal DataAreaID As String) As Boolean
        ...
        Dim NextTicketRecID As Integer
        Dim NextTicketEventRecID As Integer
        ...
        If Debug_Mode Then
            ...
            sEventEntry = "CIM Service - Function ProcessSupportMessage #01 - From: " _
                & msg.From.ToString() & " - To: " & msg.To.ToString()
            WriteToEventLog(sEventEntry, EventLogEntryType.Information)
        End If
        Try
            ...
            ElseIf (InStr(UCase(msg.Subject), "CUSTOMER INCIDENT") > 0) And ((InStr(UCase(msg.Subject), "WITNESS") > 0) Or (InStr(UCase(msg.Subject), "VERINT") > 0)) Then
                ...
                End If
        Catch ex As Exception
            sEventEntry = "CIM Service : Exception - Function ProcessSupportMessage #04, returns " _
        ...
        If (Len(Partner3_) > 0) Then ' partner integration needs to have a partner to process!
            If (PartnerTicketNo <> "") Then ' Verint integration needs to have a ticket number specified to process!
                Try
                    ...
                    If IntegrationRequestType = "NEW" Then ' only insert if this is a new ticket
                        ...
                        If Len(TicketID) = 0 Then Exit Function
                        ...
                        'C_Tickets
                        If Debug_Mode Then WriteToEventLog("CIM Service - Function ProcessSupportMessage #06 - Next Ticket ID: " & TicketID, EventLogEntryType.Information)
                        With DManager
                            ...
                            NextTicketRecID = GetNextTicketRecID()
                            If NextTicketRecID <= 0 Then Exit Function
                            SQLString = "INSERT INTO C_Tickets ( " _
                        ...
                        'C_TicketEvents
                        NextTicketEventRecID = GetNextTicketEventRecId()
                        If NextTicketEventRecID <= 0 Then Exit Function
                        With DManager
                            SQLString = "INSERT INTO C_TicketEvents (EventType, Private, TicketID, Comment_, CreatedBy, " _
                                        ...
                                        & "VALUES ('New Ticket', 0, '" & TicketID & "', 'New " & Partner3_ & " Integration Email received: " _
                            ...
                            sEventEntry = "CIM Service - Function ProcessSupportMessage #08 - Inserted Ticket ID: " & TicketID & " Event" _
                                & vbCrLf & "SQL: " & SQLString & vbCrLf
                            WriteToEventLog(sEventEntry, EventLogEntryType.Information)
                            .ExecuteSQL(SQLString)
                        End With
                    Else ' update ticket
                        ...
                        If Len(TicketID) = 0 Then ' send email to Arnett Kelly and Brian Brazil that an update request was recieved but no ticket was found internally
                ...
                Catch ex As Exception
                    ...
                    sEventEntry = "CIM Service : Exception - Function ProcessSupportMessage #19, returns " _
                        & Convert.ToString(ProcessSupportMessage) & vbCrLf _
            ...
            Else ' process other partner ticket emails (non integration)
                ...
                Catch ex As Exception
                    'Write("CIM Service", ex.Message & vbCrLf & vbCrLf & vbCrLf & "Current status(es): " & eventViewerString, 2)
                    'Write("CIM Service", ex.Message & vbCrLf & vbCrLf & "Possible SQL: " & SQLString, 2)
                    sEventEntry = "CIM Service : Exception - Function ProcessSupportMessage #22, returns " _
                        & Convert.ToString(ProcessSupportMessage) & vbCrLf _
                        & "Target: " & ex.TargetSite.ToString() & vbCrLf _
                        & "Message: " & ex.Message & vbCrLf _
                        & "Possible SQL: " & SQLString & vbCrLf
    ...
    Sub ParseVerintOnyxEmail(ByVal TicketEmailBody As String, ByVal MsgID As String, ByVal IntegrationRequestType As String)
        ...
        If (PriorityStart > 9) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Priority: " & Priority, EventLogEntryType.Information)
        ...
        If (CompanyNameStart > 13) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - CompanyName: " & CompanyName, EventLogEntryType.Information)
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - SiteName: " & SiteName, EventLogEntryType.Information)
        ...
        If (OnyxCustomerNumberStart > 21) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - OnyxCustomerNumber: " & OnyxCustomerNumber, EventLogEntryType.Information)
        ...
        If (OnyxIncidentNumberStart > 21) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - OnyxIncidentNumber: " & OnyxIncidentNumber, EventLogEntryType.Information)
        ...
        If (CustomerPrimaryContactStart > 25) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - CustomerPrimaryContact: " & CustomerPrimaryContact, EventLogEntryType.Information)
        ...
        If (LocationStart > 9) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Location: " & Location, EventLogEntryType.Information)
        ...
        If (PostalCodeStart > 12) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - PostalCode: " & PostalCode, EventLogEntryType.Information)
        ...
        If (IncidentContactStart > 17) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - IncidentContact: " & IncidentContact, EventLogEntryType.Information)
        ...
        If (BusinessPhoneStart > 15) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - BusinessPhone: " & BusinessPhone, EventLogEntryType.Information)
        ...
        If (CellPhoneStart > 11) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - CellPhone: " & CellPhone, EventLogEntryType.Information)
        ...
        If (FaxStart > 4) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Fax: " & Fax, EventLogEntryType.Information)
        ...
        If (PagerStart > 6) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Pager: " & Pager, EventLogEntryType.Information)
        ...
        If (EmailAddressStart > 14) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - EmailAddress: " & EmailAddress, EventLogEntryType.Information)
        ...
        If (SupportLevelStart > 14) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - SupportLevel: " & SupportLevel, EventLogEntryType.Information)
        ...
        If (IncidentTypeStart > 14) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - IncidentType: " & IncidentType, EventLogEntryType.Information)
        ...
        If (ProductStart > 8) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Product: " & Product, EventLogEntryType.Information)
        ...
        If (VersionStart > 8) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - Version: " & Version, EventLogEntryType.Information)
        ...
        If (SerialNumberStart > 14) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - SerialNumber: " & SerialNumber, EventLogEntryType.Information)
        ...
        If (SubmittedByStart > 13) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - SubmittedBy: " & SubmittedBy, EventLogEntryType.Information)
        ...
        If (WorkNotesStart > 10) Then
            ...
            End If
        End If
        If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOnyxEmail - WorkNotes: " & WorkNotes, EventLogEntryType.Information)
        ...
        sEventEntry = "CIM Service - Parsed an Onyx integration email Message (ID: " & MsgID & ")" & vbCrLf & "Priority: '" & Priority & "'" & vbCrLf _
                ...
                & "Serial Number: '" & SerialNumber & "'" & vbCrLf & "Submitted By: '" & SubmittedBy & "'" & vbCrLf _
                & "Severity Level: '" & SeverityLevel & "'" & vbCrLf & "Severity Level Description: '" & SeverityLevelDescription & "'" & vbCrLf _
    ...
    Sub ParseVerintOracleEmail(ByVal TicketEmailBody As String, ByVal MsgID As String, ByVal IntegrationRequestType As String)
        ...
        Try
            ...
            If (CustomerNameStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - CustomerName: " & CustomerName, EventLogEntryType.Information)
            ...
            If (SiteNameStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - SiteName: " & SiteName, EventLogEntryType.Information)
            ...
            If (CustomerNumberStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - CustomerNumber: " & CustomerNumber, EventLogEntryType.Information)
            ...
            If (IncidentNumberStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - IncidentNumber: " & IncidentNumber, EventLogEntryType.Information)
            ...
            If (LocationStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - Location: " & Location, EventLogEntryType.Information)
            ...
            If (PostalCodeStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - PostalCode: " & PostalCode, EventLogEntryType.Information)
            ...
            If (IncidentContactStart > 40) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - IncidentContact First Name: " & IncidentContact, EventLogEntryType.Information)
            ...
            If (IncidentContactStart > 40) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - IncidentContact First Name & Last Name: " & IncidentContact, EventLogEntryType.Information)
            ...
            If (IncidentPrimaryContactPhoneStart > 40) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - IncidentPrimaryContactPhone: " & IncidentPrimaryContactPhone, EventLogEntryType.Information)
            ...
            If (IncidentPrimaryContactEmailStart > 40) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - IncidentPrimaryContactEmail: " & IncidentPrimaryContactEmail, EventLogEntryType.Information)
            ...
            If (SupportLevelStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - SupportLevel: " & SupportLevel, EventLogEntryType.Information)
            ...
            If (SRTypeStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - SRType: " & SRType, EventLogEntryType.Information)
            ...
            If (ServiceProductVerticalStart > 26) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - ServiceProductVertical: " & ServiceProductVertical, EventLogEntryType.Information)
            ...
            If (SerialNumberStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - SerialNumber: " & SerialNumber, EventLogEntryType.Information)
            ...
            If (TaskStatusStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - TaskStatus: " & TaskStatus, EventLogEntryType.Information)
            ...
            If (LoggedByStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - LoggedBy: " & LoggedBy, EventLogEntryType.Information)
            ...
            If (WorkNotesStart > 21) Then
                ...
                End If
            End If
            If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail - WorkNotes: " & WorkNotes, EventLogEntryType.Information)
        ...
        If IntegrationRequestType <> "NEW" Then
            ...
            Try
                If Debug_Mode Then WriteToEventLog("CIM Service : Sub ParseVerintOracleEmail #05 IF..THEN block begins", EventLogEntryType.Information)
                If (OldCustomerName <> CustomerName) Then
    ...
    Public Function GetNextTicketRecID() As Long
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception in Function: GetNextTicketRecID, returns " _
    ...
    Public Function GetNextTicketId(ByVal DataAreaID As String) As String
        ...
        ' gets and increments
        NextTicketIdNum = GetTicketNumberSequence()
        If NextTicketIdNum > 0 Then
    ...
    Public Function GetTicketNumberSequence() As Integer
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception in Function: GetTicketNumberSequence, returns " _
    ...
    Public Function IncrementTicketNumberSequence() As Boolean
        ...
        Catch ex As Exception
            'Write("Witness Quote Process - " & "Procedure:GetTicketNumberSequence ", ex.Message, 2)
            sEventEntry = "CIM Service : Exception in Function: IncrementTicketNumberSequence, returns " _
    ...
    Public Function GetNextTicketEventRecId() As Long
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception in Function GetNextTicketEventRecId, returns " _
    ...
    Public Function FindMessageID(ByVal MessageID As String, ByRef ErrorOccurs As Boolean) As Boolean
        Dim sql As String = ""
        ...
        Try
            ...
            If FindMessageID = False Then ' if not found, then this is a NEW message
                If Debug_Mode Then
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception Function FindMessageID, returns " & Convert.ToString(FindMessageID) & vbCrLf _
                & "ErrorOccurs (passed by Ref): " & Convert.ToString(ErrorOccurs) & vbCrLf _
    ...
    Public Function UpdateMessageID(ByVal MessageID As String) As Boolean
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception Function UpdateMessageID, returns " & Convert.ToString(UpdateMessageID) & vbCrLf _
                & "Target: " & ex.TargetSite.ToString() & vbCrLf & "Message: " & ex.Message
            WriteToEventLog(sEventEntry, EventLogEntryType.Error)
        End Try
    End Function
    ...
    Private Function OpenDB() As Boolean
        ...
        Catch ex As Exception
            ...
            sEventEntry = "CIM Service : Exception Function OpenDB, returns " & Convert.ToString(OpenDB) & vbCrLf _
                & "Connecting string: " & ConnString & vbCrLf _
                & "Target: " & ex.TargetSite.ToString() & vbCrLf & "Message: " & ex.Message
            WriteToEventLog(sEventEntry, EventLogEntryType.Error)
        End Try
    End Function
    ...
    Private Function ValidateEmailSource(ByVal MsgID As String, ByVal MsgFrom As String) As Boolean
        ...
        Try
            ...
            End If
            If Debug_Mode Then
                sEventEntry = "CIM Service: Function ValidateEmailSource validates and returns " & Convert.ToString(ValidateEmailSource) & _
                    " on" & vbCrLf & "Message ID: " & msgid & vbCrLf & "Message From: " & MsgFrom
                WriteToEventLog(sEventEntry, EventLogEntryType.Information)
            End If
        Catch ex As Exception
    ...
    Private Function FindMin(ByVal first As Integer, ByVal x As Integer, ByVal y As Integer, ByVal z As Integer) As Integer
        ...
        Try
            ...
            Next iter
        Catch ex As Exception
            sEventEntry = "CIM Service : Exception Function FindMin.  The 4 parameters are " _
